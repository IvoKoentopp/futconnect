      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-white border-b shadow-sm">
        <div className="flex items-center justify-between h-16 px-4">
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navItems.map(item => item.submenu ? <div key={item.name} className="relative group">
                ...
                </div> : <Link key={item.path} to={item.path} className={`flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors ${location.pathname === item.path ? 'bg-futconnect-50 text-futconnect-700' : 'text-gray-700 hover:bg-gray-100'}`}>
                  <span className="mr-2">{item.icon}</span>
                  {item.name}
                </Link>)}
          </div>
          ...
          <div className="fixed top-16 left-0 right-0 z-40 bg-white shadow-lg overflow-y-auto max-h-[calc(100vh-4rem)]">
            <nav className="px-2 pt-2 pb-4">
              <ul className="space-y-1">
                {navItems.map(item => item.submenu ? <li key={item.name} className="space-y-1">
